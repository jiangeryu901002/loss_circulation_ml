...
from utils_contrastive import extract_non_diagonal_matrix, split_features
...
class RBF(nn.Module):
    ...
    def get_bandwidth(self, L2_distances):
        if self.bandwidth is None:
            ...
            return L2_distances.data.sum(dim=(-2,-1)) / (n_samples ** 2 - n_samples)

        return self.bandwidth

    def forward(self, X):
        L2_distances = torch.cdist(X, X) ** 2
        factor = (self.get_bandwidth(L2_distances) * self.bandwidth_multipliers)[:, None, None]
        return torch.exp(-L2_distances[:, None] / factor).sum(dim=-3)
