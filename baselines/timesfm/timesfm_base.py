...
class TimesFmBase:
  ...
  """
    ...
    def dataset_with_covariates(
        ...
        """Generate dataset from a list of time series with covariates.
    ...
    """
        ...
        inputs = [
            (target + xreg_on_context) / 2
            for target, xreg_on_context in zip(targets, xregs_on_context)
        ]

        if normalize_xreg_target_per_input:
            inputs = _renormalize(inputs, per_instance_stats)
            # xregs = _renormalize(xregs, per_instance_stats)

        # outputs = [
        #       output#(output - xreg)
        #       for output, xreg in zip(outputs, xregs)
        #   ]
