...
class FinetuningConfig:
    ...
    checkpoint_save_path: str = 'checkpoint'
...
class TimesFMFinetuner:
    ...
    def _process_batch(self, batch: List[torch.Tensor]) -> tuple:
        ...
        context_len, horizon_len = x_context.shape[-1], x_future.shape[-1]
        # x_future = x_future[:, channel_id]
        predictions = self.model(x_context, x_padding.float(), freq)
        # predictions_mean = predictions[..., 0]
        # last_patch_pred = predictions_mean[:, -1, :]
        predictions_mean = predictions[..., context_len:context_len + horizon_len, 0]  # [B, N, horizon_len]
        last_patch_pred = predictions_mean[:, -1, :]  # [B, horizon_len]
        # print('batch:', last_patch_pred.shape, x_future.squeeze(-1).shape)
        ...
        if self.config.use_quantile_loss:
            ...
            for i, quantile in enumerate(quantiles):
                last_patch_quantile = predictions[:, -1, context_len:context_len + horizon_len, i + 1]
                loss += torch.mean(
        ...
        try:
            for epoch in range(self.config.num_epochs):
                ...
                if val_loss < best_val_loss:
                    ...
                    torch.save(self.model.state_dict(), f'{self.config.checkpoint_save_path}.pt')
